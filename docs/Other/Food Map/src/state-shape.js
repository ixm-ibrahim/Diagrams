/* Initial shape of the global state observable.
 *
 * Split out of state.js so the observable pattern (subscribe / set / get)
 * stays small and the field-by-field shape lives in one scannable place.
 * Every field carries a comment explaining its role and origin phase so
 * future additions follow the same convention. Field values here are
 * the initial values seeded at boot; main.js may overwrite many of them
 * once the dataset loads.
 */
...
export const INITIAL_STATE = {
  ...
  // Phase 40 round 4 / Batch 4 rework: global Food groups filter.
  // Tri-state per food_group, matching the categories filter:
  //   included: aggregate/ingredient must reference at least one of
  //             these (with match-all on: ALL of them, at meal view).
  //   excluded: hide anything that references any of these.
  // Empty arrays = no filter.
  foodGroupFilter: { included: [], excluded: [] },
  // Batch 4: AND/OR + ANY/ALL controls for the food-group filter.
  // Only meaningful at meal view (single-group items collapse the
  // semantic); the UI hides the toggles at non-meal views.
  foodGroupFilterMatch: 'any', // 'any' | 'all'
  foodGroupFilterScope: 'any', // 'any' | 'all'
  ...
  // Tester-feedback Batch 4: which axis the right-rail Remix section edits —
  // 'category' (swap broad ingredient_categories) or 'ingredient' (swap the
  // meal's specific example_ingredients). One section, toggled. Persisted as a
  // UI preference. The mealDraft above carries either a `categories` list or an
  // `ingredients` (id) list depending on the active mode.
  remixMode: 'ingredient',
